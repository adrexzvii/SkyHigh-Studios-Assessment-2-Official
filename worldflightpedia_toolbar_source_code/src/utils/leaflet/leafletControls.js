 ...
 */
export function addFollowControl(
  map,
  { palette, followRef, setFollowPlane, updateFollowButtonRef }
) {
 ...
 * @returns {L.Control} The created Leaflet control.
 */
export function addPausePlayControl(
  map,
  { SimVar, pauseRef, updatePauseButtonRef, pauseBlinkIntervalRef }
) {
  const PausePlayControl = L.Control.extend({
    ...
    onAdd: function () {
      ...
      const renderIcon = () => {
        ...
        root.render(
          React.createElement(isPaused ? PlayArrowIcon : PauseIcon, {
            style: { fontSize: "18px", color: "#000" },
          })
        );
      ...
      const applyBlinking = () => {
        ...
        if (isPaused) {
          let green = true;
          btn.style.backgroundColor = "#00E46A";
          pauseBlinkIntervalRef.current = setInterval(() => {
            green = !green;
            btn.style.backgroundColor = green ? "#00E46A" : "#fff";
          }, 1000);
        } else {
        ...
        try {
          ...
          try {
            window?.localStorage?.setItem(
              "wfp_map_pause",
              JSON.stringify(Boolean(newPaused))
            );
          } catch (_) {}
        } catch (err) {
  ...
  const ZoomControl = L.Control.extend({
    ...
    onAdd: function () {
      ...
      L.DomEvent.on(zoomInBtn, "mouseenter", () => {
        ...
        zoomInRoot.render(
          React.createElement(AddIcon, {
            style: { fontSize: "20px", color: palette?.dark || "#000" },
          })
        );
      });
