/**
 * HelpOverlay - Fullscreen help overlay used by the toolbar
 *
 * Renders a centered Card containing an illustrative help image and a Close
 * action. The overlay is intended to be visually similar to Leaflet popups
 * (white content, subtle border/shadow and a compact scrollbar). When the
 * app runs inside MSFS/CoherentGT (COUI) a COUI-hosted image path can be
 * provided via the global `window.__COUI_HELP_IMAGE`. A fallback image from
 * the `public/` folder is used otherwise.
 *
 * Props:
 * - open {boolean}: whether the overlay is visible
 * - onClose {Function}: callback invoked when the overlay requests to close
 */
import {
  Card,
  CardContent,
        ...
        >
          ...
          */}
          {(() => {
            ...
            const couiSrc =
              typeof window !== "undefined" && window.__COUI_HELP_IMAGE
